...
class AuditCustomFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        if has_request_context() and hasattr(request, "transaction_id"):
            record.msg = f"{request.transaction_id} - {record.msg}"
...
def configure_log_level(level):
    ...
    for logger in loggers:
        ...
        if logger_name.startswith('payment'):
...
def configure_logging(environ):
    ...
    configure_logger_for_environ('payment')
